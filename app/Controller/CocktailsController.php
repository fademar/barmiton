...
class CocktailsController extends Controller
{
	...
	public function showcocktails() {
		...
		$urlcouleurs		= $cocktails->constructUrl('/colored/' . $_couleur['champuk']);
		$_cocktailscouleur 	= $cocktails->getCocktailListBy($urlcouleurs);
		$_cocktailscouleur 	= $cocktails->getRandomCocktail($_cocktailscouleur['list'], 4);

		$urloccasions		= $cocktails->constructUrl('/for/' . $_occasion['champuk']);
		$_cocktailsoccasion = $cocktails->getCocktailListBy($urloccasions);
